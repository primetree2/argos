...
export function ChallengeLobby({ challenges }: { challenges: OpenChallenge[] }) {
    ...
    const handlePost = async () => {
        if (!topic.trim()) {
            setPostError("Enter a topic to post a challenge.");
            return;
        }
        setPosting(true);
        setPostError("");
    ...
    const handleAccept = async (id: string) => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) {
                setAcceptError(data.error ?? "Could not accept this challenge.");
                setAcceptingId(null);
                router.refresh();
                return;
            }
            ...
            setAcceptError("The Oracle is unreachable. Try again.");
            setAcceptingId(null);
        }
    };
    ...
    return (
        <>
            ...
            <div className="reveal-2 glass-card glass-card-gold" style={{ padding: "1.5rem", marginBottom: "2.5rem" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "1rem" }}>
                    <button onClick={handlePost} disabled={posting} className="btn-oracle" style={{ fontSize: "0.7rem", letterSpacing: "0.18em", padding: "0.7rem 1.5rem" }}>
                        {posting ? (
                            <><span style={{ animation: "oracle-pulse 1s ease-in-out infinite" }}>◆</span>&nbsp;Posting…</>
                        ) : "Post to Lobby →"}
                    </button>
                </div>
            </div>
            ...
            <div className="reveal-3">
                <p style={{ fontFamily: "var(--font-cinzel), serif", fontSize: "0.6rem", letterSpacing: "0.22em", color: "var(--text-gold)", opacity: 0.9, textTransform: "uppercase", marginBottom: "1rem" }}>
                ...
                {challenges.length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
                        {challenges.map((c) => (
                            <article key={c.id} className="glass-card" style={{ padding: "1.25rem 1.4rem", display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
